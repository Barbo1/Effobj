...
static nodeWS copy_nodes(nodeWS node, bool first = true){
    if(node != nullptr){
        ...
        node_new->sibling = (first ? nullptr: copy_nodes(node->sibling, false));
...
WordSet::iterator& WordSet::iterator::operator++(){
    ...
    if (tr->son != nullptr) {
        ...
        while (_large_ >= 0 && _nodes_[_large_]->sibling == nullptr) _large_--;
        if (_large_ < 0) return *this;
...
unsigned WordSet::delete_word(const std::string & word) {
    ...
    if (n <= MAX_LEN_WORD) {
        ...
        if (tr_father->finished && str == fin) {
            ...
            while (i != -1 && !tr->finished && tr->son == nullptr && tr->sibling == nullptr && tr_father->son == tr) {
...
WordSet WordSet::operator!() const {
    ...
    for (it = WordSet::iterator(*this); !it.is_finished(); ++it) {
        ...
        l = le>>1;
        le--;
        for (i = 0; i < l; i++) std::swap(str[le-i], str[i]);
...
WordSet from_file_to_set(const std::string & path){
    ...
    if (file.is_open()) {
        ...
        std::string word = "";
        for (char i = 0, lett = text[0]; i < len; i++, lett = text[i]) {
            if (lett != ' ' && lett != '\n') [[likely]] {
                word += lett;
            } else [[unlikely]] {
                if (word.length() > 0) ws.aggregate_word(word);
                word = "";
            }
        }
        delete [] text;
