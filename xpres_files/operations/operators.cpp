...
Xpres& Xpres::operate_operator(const char oper, Xpres& xp, const Xpres& exp) {
  nodeEX prin = copy_nodes(exp.root);

  std::vector<uint64_t> vecc;
  const uint64_t n = xp.token_info.size();
  uint64_t cont = n, i;
  for (const auto& [name, data1, data2]: exp.token_info) {
    i = 0;
    while (i < n && std::get<0>(xp.token_info[i]) != name) {
      i++;
    }
    if (i == n) {
      xp.token_info.push_back ({name, data1, data2});
      vecc.push_back (cont++);
    } else {
      vecc.push_back (i);
    }
...
void Xpres::iterator::almost_empty_points(std::stack<nodeEX>& points) {
  if (points.size() > 1) {
    points.pop();
  }
}

void Xpres::iterator::operate_operator_iterator(const char oper, Xpres::iterator& it, const Xpres& exp) { 
